...
public class TokenRequestHandler implements MessageHandler {
	...
	public void run() {
		if (clientMessage.getMessageType() == MessageType.TOKEN_REQUEST) {
			...
			TokenRequest message = (TokenRequest)clientMessage;
...
//				Proverim da li imam token i algoritam iz knjige.
				
			}else {
				AppInfo.timestampedStandardPrint("This token request message is not ment for me.");

//				Nije meni upucena poruka. Nadji najpodobnijeg za dalje slanje
				NodeInfo receiver = checkTarget(recieverId);
				if(receiver != null) {
					AppInfo.timestampedStandardPrint("Receiver for this token request message should be " + receiver.getId());
//					Nasli smo najboljeg, salji njemu
					TokenRequest requestMessage = new TokenRequest(originalSender, AppInfo.myInfo, recieverId, receiver);
					MessageUtil.sendMessage(requestMessage);
				}else {
//					Doslo je do greske
					AppInfo.timestampedErrorPrint("Greska pri pronalazenju najboljeg za dalje slanje token request poruke");
				}
